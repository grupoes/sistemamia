...
export const mensajes_numero = async (req, res) => {
  ...
  try {
      ...
      where: {
        id: {
          [Op.in]: sequelize.literal(
            `(SELECT id FROM chat WHERE "from"='${numero}' OR receipt='${numero}' ORDER BY timestamp DESC)`
          ),
        },
      },
      order: [["timestamp", "ASC"]],
    ...
    if (mensajes.length === 0) {
      return res
        .status(404)
        .json({ message: "No_hay", data: "No se encontraron mensajes" });
    }

    for (let mensaje of mensajes) {
      if (mensaje.idRes) {
        const mensajeIdRes = await Chat.findOne({
          where: { codigo: mensaje.idRes },
        });
        if (mensajeIdRes) {
    ...
    return res
      .status(500)
      .json({ message: "error", data: "Error interno del servidor." });
  }
};
  ...
  try {
    ...
    for (const contacto of contactos) {
      const ultimoChat = await Chat.findOne({
        where: {
          [Op.or]: [{ from: contacto.from }, { receipt: contacto.from }],
        },
        order: [["id", "DESC"]],
      });
...
export const uploadImage = async (req, res, next) => {
  ...
  uploadSingle(req, res, async (error) => {
    ...
    if (
      ...
      ar.mimetype == "image/png" ||
      ar.mimetype == "image/gif" ||
      ar.mimetype == "image/webp" ||
      ar.mimetype == "image/svg+xml"
    ) {
      url_imagen =
        process.env.URL_APP +
        ":" +
    ...
    if (
      ...
      ar.mimetype ==
        "application/vnd.openxmlformats-officedocument.wordprocessingml.document" ||
      ar.mimetype == "application/pdf" ||
      ar.mimetype == "text/xml" ||
      ar.mimetype == "application/x-zip-compressed" ||
      ar.mimetype == "application/octet-stream" ||
      ar.mimetype == "text/plain" ||
      ar.mimetype ==
        "application/vnd.openxmlformats-officedocument.presentationml.presentation"
    ) {
      url_imagen =
...
export const asignarClienteAUnTrabajador = async (req, res) => {
  ...
  try {
    ...
    if (existeChat == 1) {
      ...
      const updateWhatsapp = await NumeroWhatsapp.update(
...
export const uploadAudio = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      return res.json({
        mensaje: "ok",
        subido: "Archivo subido con éxito.",
        datos: dataFile,
        api: datos,
        newMensaje: new_message,
      });
    } catch (error) {
      console.error(
        "Error in making request:",
...
export const asignarAsistente = async (req, res) => {
  ...
  try {
    console.log(numero);
    const actualizar = await NumeroWhatsapp.update(
      { asistente: asistente },
      {
        where: { from: numero },
      }
...
export const contactosNoContestados = async (req, res) => {
  try {
    ...
    for (let contacto of contactos) {
      ...
      const chat = await Chat.findOne({
        where: {
          [Op.or]: [{ from: numero }, { receipt: numero }],
        },
        ...
        if (chat.typeMessage === "text") {
          if (chat.from === numero) {
            ...
            if (tiempo.dias > 0 || tiempo.minutos > 3) {
              ...
              const arrayDeContenidos = frases.map((frase) =>
                frase.descripcion.toLowerCase()
              );
              ...
              const verificado = arrayDeContenidos.some((elemento) =>
                buscar.includes(elemento)
              );
...
export const delete_contacto = async (req, res) => {
  ...
  try {
    ...
    const registro = await registroActividad(
      id,
      tipo_actividad,
    ...
    return res.json({
      message: "ok",
      data: "Se elimino correctamente el contacto",
    });
  } catch (error) {
    return res.json({ message: error.message });
  }
...
function contienePalabra(frase, palabras) {
  const palabrasFrase = new Set(frase.toLowerCase().split(" "));

  return palabras.some((palabra) => palabrasFrase.has(palabra));
}
...
//funcion para filtrar por fechas las interacciones
export const interaccionesContacto = async (req, res) => {
  ...
  try {
    const query = `
        ...
        c."from", 
        nw."nameContact",
        (SELECT STRING_AGG(e.descripcion, ', ')
         FROM etiquetas e
         INNER JOIN "etiquetaCliente" ec ON ec.etiqueta_id = e.id
         WHERE ec.cliente_id = pc.id AND ec.estado = 1) AS etiqueta
        FROM chat c
        INNER JOIN numeros_whatsapp nw ON nw."from" = c."from"
        INNER JOIN potencial_cliente pc ON pc.numero_whatsapp = nw."from"
        WHERE DATE(c."createdAt") BETWEEN '${fecha_inicio}' AND '${fecha_fin}'
        GROUP BY c."from", nw."nameContact", pc.id;
        `;
...
export const listChat = async (req, res) => {
  try {
    ...
    let etiqueta = req.params.idEtiqueta;
    let plataforma = req.params.idPlataforma;
    ...
    SELECT 
        ...
        MAX(CASE
                WHEN c."timestamp" = subconsulta.ultimo_timestamp THEN nw."nameContact"
                ELSE NULL
            END) AS nombre_contacto,
        MAX(CASE
                WHEN c."timestamp" = subconsulta.ultimo_timestamp THEN c.message
                ELSE NULL
            END) AS ultimo_mensaje,
        MAX(CASE
                WHEN c."timestamp" = subconsulta.ultimo_timestamp THEN c."typeMessage"
                ELSE NULL
            END) AS typeMessage,
        MAX(CASE
                WHEN c."timestamp" = subconsulta.ultimo_timestamp THEN c."codigo"
                ELSE NULL
            END) AS codigo,
        MAX(CASE
                WHEN c."timestamp" = subconsulta.ultimo_timestamp THEN c."from"
                ELSE NULL
            END) AS envio,
        MAX(CASE
                WHEN c."timestamp" = subconsulta.ultimo_timestamp THEN nw."asistente"
                ELSE NULL
            END) AS idasistente,
        MAX(CASE
                WHEN c."timestamp" = subconsulta.ultimo_timestamp THEN concat(t."nombres", ' ',t."apellidos")
                ELSE NULL
            END) AS asistente,
        (SELECT COUNT(*)
        FROM chat
        WHERE "from" = subconsulta.contacto AND "estadoMessage" = 'sent') AS total_mensajes_sent
    FROM (
        SELECT 
            CASE
                WHEN c."from" = '51938669769' THEN c.receipt
                ELSE c."from"
            END AS contacto,
            MAX(c."timestamp") AS ultimo_timestamp
        FROM chat c
        GROUP BY 
            CASE
                WHEN c."from" = '51938669769' THEN c.receipt
    ...
    JOIN chat c ON 
        (subconsulta.contacto = c."from" OR subconsulta.contacto = c.receipt) AND
        subconsulta.ultimo_timestamp = c."timestamp"
    JOIN numeros_whatsapp nw ON subconsulta.contacto = nw."from"
    JOIN trabajadores t ON t.id = nw.asistente
    ${sql} ${conector}
    ${sqlEtiqueta} ${conecPl}
    GROUP BY subconsulta.contacto, subconsulta.ultimo_timestamp
...
export const searchChatList = async(req, res) => {
    ...
    try {
        ...
        join numeros_whatsapp nw on subconsulta.contacto = nw."from"
        JOIN trabajadores t ON t.id = nw.asistente
        where nw."nameContact" ilike '%${search}%' ${sql}
        GROUP BY subconsulta.contacto, subconsulta.ultimo_timestamp
        ORDER BY subconsulta.ultimo_timestamp DESC;
        `;
        ...
        select (CASE
            ...
            ELSE nw."from"
        END) AS contacto,(CASE
            WHEN c."from" = '51938669769' THEN (select nw2."nameContact" from numeros_whatsapp nw2 where nw2."from" = c.receipt)
            ELSE nw."nameContact"
        END) AS nombre_contacto, c.message as ultimo_mensaje, c."timestamp" as ultimo_timestamp, c.codigo,
        concat(t.nombres, ' ', t.apellidos) as asistente, 0 AS total_mensajes_sent, c."typeMessage" 
        from chat c 
        left join numeros_whatsapp nw on nw."from" = c."from"
        right join trabajadores t on t.id = nw.asistente
        where c.message ilike '%${search}%' ${sql}
