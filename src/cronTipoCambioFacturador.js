...
const logMessage = (message) => {
  ...
  fs.appendFileSync("bot-log-tipo-cambio-facturador.txt", logEntry);
  console.log(logEntry.trim());
};

cron.schedule(
  process.env.CRON_SAVE_TIPO_CAMBIO_FACTURADOR,
  async () => {
    try {
      let config = {
        method: "get",
        maxBodyLength: Infinity,
        url: process.env.URL_INSERT_TIPO_CAMBIO_FACTURADOR,
      };
